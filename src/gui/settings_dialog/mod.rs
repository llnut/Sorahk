...
use helpers::{
    BUTTON_TEXT_MAX_CHARS, calculate_mouse_direction, estimate_arrow_width, estimate_pill_width,
    estimate_target_pill_width, get_capture_mode_display_name, get_sequence_key_color,
    get_sequence_key_display, get_target_key_color, is_mouse_move_target, is_mouse_scroll_target,
    truncate_text_safe,
};

/// Resets capture mode and clears all per-session sequence scratch state.
///
/// A macro rather than a `&mut self` method so it can be invoked inside
/// closures that already hold a disjoint sub-field borrow such as
/// `temp_config = self.temp_config.as_mut()`. Each expanded field access
/// is tracked independently under Rust's split-borrow rules, while a
/// method call would require a full mutable borrow of `self`.
...
impl SorahkGui {
    ...
    pub(super) fn render_settings_dialog(&mut self, ctx: &egui::Context) {
        ...
        if self.key_capture_mode != KeyCaptureMode::None && self.hid_activation_dialog.is_none() {
            ...
            // Sequence mode uses a configurable finalize key (default Enter)
            // to end recording without itself joining the sequence. Moving to
            // a "Done" button would force extra mouse motion into the trace.
            ...
            if let Some(input_name) = captured_input {
                ...
                // Stay in capture mode for every sequence variant so the user
                // can keep adding keys without clicking Capture again.
        ...
        egui::Window::new("")
            ...
            .show(ctx, |ui| {
                ui.push_id("settings_dialog_scope", |ui| {
                    // `CachedTranslations` is a `Copy` newtype over a
                    // `&'static` pointer. Binding by value drops the borrow on
                    // `self.translations` so the closure can still call
                    // `&mut self` helpers such as `finalize_sequence_capture!`.
                                ...
                                .show(ui, |ui| {
                                    ...
                                    egui::Frame::NONE
                                        ...
                                        .show(ui, |ui| {
                                            ...
                                            // Two-column grid keeps the capture buttons aligned
                                            // across rows regardless of per-language label
                                            // width. Matches the Global Configuration section
                                            // below.
